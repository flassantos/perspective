...
class Add extends Controller{
	...
	public function index_index(){
		...
		if(count($ver) > 0){
			...
			else if ($nomePost == "patrick-jane"){
				$status = 1;
			}
			$add->insereDados(array("id_usuario"=>$id, "id_amigo"=>$idAtual, "status"=>$status));	
	...
	public function delete(){
		...
		if(count($ver) > 0){
			...
			$add->_tabela = "amizades";
			if ($return == 'delete'){
				$add->deletaDados("status=? AND id_usuario=? AND id_amigo=?", array("status"=>1, "id_usuario"=>$idAtual, "id_amigo"=>$idAmigo));
				$add->deletaDados("status=? AND id_usuario=? AND id_amigo=?", array("status"=>1, "id_usuario"=>$idAmigo, "id_amigo"=>$idAtual));
				$return = 'true';
			}
		}
