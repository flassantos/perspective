...
class ErrorPag extends Controller{
	...
	public function index_index(){
		...
		$res6 = $errorm->listaDados("id, nome, nome_url, foto, cidade_uf", "id>?", array("id"=>0), "nome");

		$errorm->_tabela = "amizades";
		$res7 = $errorm->listaDados("id_usuario, id_amigo", "(id_amigo=? OR id_usuario=?) AND status=?", array("id_amigo"=>$idGlobal, "id_usuario"=>$idGlobal, "status"=>1));
