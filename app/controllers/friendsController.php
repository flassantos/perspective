...
class Friends extends Controller{
	...
	public function index_index(){
		...
		$res6 = $friends->listaDados("id, nome, nome_url, foto, cidade_uf", "id>?", array("id"=>0), "nome");
		
		$res = $friends->listaDados("id, nome_url, nome, email, data_nascimento, telefone, cidade_uf, foto, fundo", "$col=?", array($col=>$var));

		$friends->_tabela = "amizades";
		$res7 = $friends->listaDados("id_usuario, id_amigo", "(id_amigo=? OR id_usuario=?) AND status=?", array("id_amigo"=>$idGlobal, "id_usuario"=>$idGlobal, "status"=>1));
