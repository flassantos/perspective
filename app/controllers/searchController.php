...
class Search extends Controller{
	...
	public function index_index(){
		...
		$res6 = $search->listaDados("id, nome, nome_url, foto, cidade_uf", "id>?", array("id"=>0), "nome");

		$res = $search->listaDados("id, nome_url, nome, email, data_nascimento, telefone, cidade_uf, foto, fundo", "id=?", array("id"=>$idAtual));
		
		$search->_tabela = "amizades";
		$res7 = $search->listaDados("id_usuario, id_amigo", "(id_amigo=? OR id_usuario=?) AND status=?", array("id_amigo"=>$idGlobal, "id_usuario"=>$idGlobal, "status"=>1));
