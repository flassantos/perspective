...
class ajaxModel extends Model{
	...
	public function inserirMensagens($tipo, $id_user, $id_amigo, $conteudo){
		$this->_tabela = $tipo;
	...
	public function carregarPost($qtd, $sobreid=null){
		$inicio = $qtd;
		$final = $qtd + 5;
		$limit = $inicio.",".$final;
		$idAtual = $_SESSION['loginId'];
		$iduser = ($sobreid == null) ? $idAtual : $sobreid;

		$first = new indexModel();
		$first->_tabela = "publicacoes";
		$res5 = $first->read("id, id_usuario, id_amigo, conteudo, num_curtir, data", "id_usuario=? AND status<=?", array("id_usuario"=>$iduser, "status"=>1), "id DESC", $limit);
		$first->_tabela = "usuarios";
		$res5 = $first->organizaDados($res5, array("nome_usuario", "nome_usuario_url", "foto_usuario", "comentarios"), $iduser);
...
if($_GET['tipo'] == "mensagens"){
	...
	elseif(isset($_GET['acao']) && $_GET['acao'] == "inserir"){
		$ajax->inserirMensagens($_GET['tipo'], $_GET['id_user'], $_GET['id_amigo'], $_GET['conteudo']);	
	}
}
	...
	session_start();
	$sobreid = isset($_GET['sobreid']) ? $_GET['sobreid'] : null; 
	$view_postagens = $ajax->carregarPost($_GET['qtd'], $sobreid);
